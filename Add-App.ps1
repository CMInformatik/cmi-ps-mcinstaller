function Add-App {
    ...
    Process {
        ...
        foreach($tenant in $TenantName){
            ...
            if(-not $data.$tenant.common){
                Write-Error "Tenant $tenant does not have required common configuration" -TargetObject $tenant
                continue
            }
            if(-not $data.$tenant.common.appDirectory){
