...
namespace cmi.mc.config.ModelContract
{
    ...
    public static class McSymbols
    {
        ...
        static McSymbols()
        {
            ...
            foreach (var app in Apps) ConfigurationNameToApp.Add(app.ToConfigurationName(), app);
        }
        ...
        /// </summary>
        public static IList<ConfigControlAttribute> ConfigControlAttributes { get; } =
            (ConfigControlAttribute[]) Enum.GetValues(typeof(ConfigControlAttribute));
        ...
        {
            if (ConfigurationNameToCca.ContainsKey(configurationName)) return ConfigurationNameToCca[configurationName];
            throw new ArgumentException(
                $"{configurationName} is not a configuration name for any {nameof(ConfigControlAttribute)}",
        ...
        public static Platform GetPlatform(string configurationName)
        {
            ...
            throw new ArgumentException($"{configurationName} is not a configuration name for any {nameof(Platform)}",
                nameof(configurationName));
        }
